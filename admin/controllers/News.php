...
class News extends Admin_Controller
{
  ...
  public function index()
  {
    ...
    $str ='<a href="'.site_url('news/add/{id}').'" class="btn btn btn-padding"><i class="fa fa-edit edit"></i></a><a href="javascript:void(0);" data-original-title="Remove" data-toggle="tooltip" data-placement="top" class="table-action btn-padding" onclick="delete_record(\'news/delete/{id}\',this);"><i class="fa fa-trash-o trash"></i></a>';    
    $this->listing->initialize(array('listing_action' => $str));
    
    $listing = $this->listing->get_listings('news_model', 'listing');
    
    $this->data['btn'] = "";
    $this->data['btn'] ="<a href=".site_url('news/add')." class='btn green'>Add New <i class='fa fa-plus'></i></a>";
    ...
    $this->layout->view("frontend/news/index");
  }
  ...
  public function add($edit_id = '')
  {
       ...
       try
        ...
        catch (Exception $e)
        {
          ...
          {
              $this->data["editdata"] = array("title"=>"","description"=>"","author"=>"","status"=>"",'image_name'=>'',);
    ...
    public function do_upload()
    {
        $this->load->library('upload');
                ...
                $config['upload_path']   = '../admin/uploads/news/';
  ...
  function delete($del_id)
   {
      ...
      $access_data = $this->news_model->get_where(array("id"=>$del_id),'id')->row_array();  
       
      $output=array();
      if(count($access_data) > 0){

          $image_data = $this->news_model->get_image_by_id($del_id);

          $path = getcwd() . '/uploads/news/';

          delete_file($path, $image_data[0]->image_name);

          $this->news_model->delete(array("id"=>$del_id));
