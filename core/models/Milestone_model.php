...
class Milestone_model extends App_model
{
  ...
  {
    $this->db->select("a.*,b.name as m_name,c.work_name as w_name,b.start_date,b.end_date,d.project_name,e.room_name");
    $this->db->from("milestone_status a");
    $this->db->join("project_milestones b","a.milestone_id=b.id");
    $this->db->join("work_items c","a.work_item=c.id");
    $this->db->join("project d","b.project_id=d.id");
